       ...
       setTrashNotes : (state, {payload}) => {
        ...
        setPinnedNotes : (state, {payload}) => {
            state.mainNotes = state.mainNotes.map(
                (note) => note.id !== payload.id ? {...note, isPinned : !note.isPinned} : note
            )
        },
        ...
        readNote : (state, {payload}) => {
            ...
            const setRead = (notes : noteType) => {
                state[notes] = state[notes].map((note:Note) => note.id === id ? {...note, isRead: !note.isRead} : note)
            };
            if (type === "archive") {
